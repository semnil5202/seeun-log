...
- [ ] PlaceInfoCard 가격대: `price_min`/`price_max` (integer, 만원 단위) → 표시 시 `-` 구분자 + 만원 단위 텍스트 적용
- [ ] 만원 단위 다국어 번역 키 추가 (`translations.ts`에 `place.priceUnit` 등)
- [ ] 가격대 포맷팅 유틸 함수 구현 (locale별 단위 표시: 만원 / 10K KRW / 万ウォン 등)
...
- [ ] Client: PostTranslation, LocalizedPost 타입에 place_name, address / translated_place_name, translated_address 추가
- [ ] Client: getLocalizedPost()에서 translated_place_name, translated_address 매핑
- [ ] Client: mock 번역 데이터에 place_name, address 추가
- [ ] Client: translations.ts에 place.* 번역 키 추가 (category, name, address, price, copyOriginal)
- [ ] Client: PlaceInfoCard — 번역 표시, 장소명 복사 버튼 추가, 필드 라벨 다국어 처리, 복사 스크립트 변경
- [ ] Client: PostLayout — PlaceInfoCard에 translatedPlaceName, translatedAddress props 전달
- [ ] Client: PostCard, SponsoredCard — translated_place_name 우선 표시
- [ ] Client: search-data.ts — 번역 장소명으로 검색/추천 키워드
- [ ] Client: feed JSON API — 번역 장소명 사용
- [ ] Admin: GPT-4o 번역 파이프라인에 place_name, address 포함 (Phase 4+ 구현 시)
