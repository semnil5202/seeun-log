...
| 번역        | OpenAI GPT-5 Mini (브라우저 직접 호출). 번역+요약+용어 추출+슬러그+카테고리 번역. 프롬프트: `shared/constants/prompts.ts` |
...
| 제약                       | 이유                               | 대응                                                                 |
| -------------------------- | ---------------------------------- | -------------------------------------------------------------------- |
| Vercel Hobby 10초 타임아웃 | Serverless Function 실행 시간 제한 | GPT 호출을 브라우저에서 직접 수행 (Server Action 타임아웃 제약 회피) |
...
/categories                → 카테고리 관리 (검색 + 그룹 테이블) — 구현 완료 (DB 연동)
/categories/new            → 카테고리 생성 (대분류/소분류) — 구현 완료 (DB 연동)
...
└── SidebarLayout (app/sidebar-layout.tsx — 'use client')
    ...
    └── SidebarInset > main
        ...
        ├── /categories     → CategoryListPage (카테고리 관리) — 구현 완료 (DB 연동)
        ├── /categories/new → NewCategoryPage (카테고리 생성) — 구현 완료 (DB 연동)
...
**모바일 사이드바 동작**: 모바일에서 네비게이션 항목 클릭 후 라우트 변경 시 `setOpenMobile(false)`로 사이드바 자동 닫기 (기존 Sheet slide-out 애니메이션 보존). side="left" Sheet는 우→좌 스와이프로도 닫기 가능.
...
- 장소명 (`place_name`)
- 주소 (`address`)
- 가격 접두어 (`price_prefix`) + 가격 (`price`) — `PriceInputRow` 공용 컴포넌트 (50/50 너비)

**제품 리뷰 전용 필드** (`product-review` 선택 시에만 표시):

- 제품 목록 — `useFieldArray`로 동적 추가/삭제 (`ProductReviewFields` 컴포넌트)
  - 제품명 (`product_name[]`)
  - 구매처 (`purchase_source[]`) + 구매 링크 (`purchase_link[]`) — 50/50 너비
...
[체험 방문 전용 필드] ← formType === 'visit' 일 때만 표시
  ├── 장소 (text input)
  ├── 주소 (text input)
  └── 가격 (PriceInputRow: 가격 설명 + 금액, 50/50 너비)
    ↓
[제품 리뷰 전용 필드] ← formType === 'product-review' 일 때만 표시
  └── 제품 목록 (useFieldArray 동적 행)
      ├── 제품명 (text input)
      ├── 구매처 + 구매링크 (50/50 너비)
...
- 레이아웃 순서: 폼 형식 → 썸네일 → 본문(제목+에디터) → 카테고리 → [체험방문 전용 필드] / [제품리뷰 전용 필드] → 3줄 요약 → 액션 버튼
- 본문 영역: 제목 입력 (`text input`, 40자 제한, 글자수 카운터) + `<Separator />` + `TiptapEditorContainer`
- 체험 방문 전용 필드: `VisitFields` 컴포넌트 (폼 형식이 `visit`일 때만 렌더링)
- 제품 리뷰 전용 필드: `ProductReviewFields` 컴포넌트 (폼 형식이 `product-review`일 때만 렌더링, `useFieldArray` 동적 행)
- 3줄 요약: textarea + "요약 생성" 버튼 (브라우저 직접 GPT 호출 `streamSummary` -- GPT-5 Mini API 연동 완료)
- 액션 버튼: "번역본 생성하기" (카테고리 선택 + 미번역 시) / "용어 검토 계속하기" (Sheet 닫은 후 재오픈) / "번역본 확인하기" (Languages 아이콘, 번역 완료 시) / "작성 완료"/"수정 완료" (Check 아이콘). 모바일: `grid grid-cols-2`, PC: `flex` 레이아웃
...
| 번역 (GPT-5 Mini)                    | 브라우저 직접 호출 (언어별 병렬)                         | Vercel 타임아웃 회피, `Promise.allSettled` 병렬 처리 |
...
├── configs/
│   ├── tiptap-extensions.ts         # ✅ Extension 설정 (CustomStarterKit, Heading, Link, Underline, CustomResizableImage, CustomLinkBookmark 등)
│   ├── image.ts                     # ✅ CustomResizableImage 확장 (DOM NodeView, 4코너 리사이즈 핸들)
│   ├── image-carousel.ts           # ✅ 이미지 캐러셀 설정
│   └── link-bookmark.ts            # ✅ CustomLinkBookmark 확장 (OG 태그 북마크 카드, DOM NodeView)
├── components/
...
│   ├── LinkPastePopup.tsx           # ✅ URL 붙여넣기 시 "링크" / "북마크 카드" 선택 팝업
...
│   ├── ProductReviewFields.tsx      # ✅ 제품 리뷰 전용 필드 (useFieldArray 동적 제품 목록)
│   ├── PriceInputRow.tsx            # ✅ 가격 설명 + 금액 입력 공용 컴포넌트 (visit/product 공용)
...
├── lib/
│   └── image.ts                     # ✅ toWebP() → WebPResult ({ blob, width, height }) — WebP 변환 + 워터마크 + 리사이징 (Canvas API, maxWidth 옵션)
├── api/
│   ├── actions.ts                   # ✅ Server Actions (checkSlugDuplicate)
│   └── client.ts                   # ✅ 브라우저 직접 GPT 호출 (streamSummary, fetchSlugSuggestions)
├── containers/
...
├── hooks/
│   └── useImageUpload.ts      # 업로드 로직 (검증 → Pre-signed URL → S3 PUT → UploadImageResult { url, width, height } 반환)
├── api/
...
[브라우저]                                                     [OpenAI API]
    ...
    │                                                                │
    ├─ 5. (실패 locale 재시도)                                       │
    │     fetchRetrySingleLocale() ── 단일 locale 재번역 ──────────>│
    │                              │                                  │
...
> **상세 프롬프트 설계서**: [`docs/gpt-prompts.md`](gpt-prompts.md) — 요약, 슬러그, 용어 추출, 본문 번역, 카테고리 번역 5가지 프롬프트의 시스템/유저 메시지, 응답 포맷, 제약 조건을 정의. 소스: `shared/constants/prompts.ts`.
...
| 작업                                      | 처리 위치          | 이유                                   |
| ----------------------------------------- | ------------------ | -------------------------------------- |
| GPT API 호출 (GPT-5 Mini)                 | 브라우저 직접 호출 | Vercel 타임아웃 회피, 언어별 병렬 처리 |
...
- **데이터**: `fetchCategories()` Server Action으로 DB 직접 조회 (`categories` 테이블 + `posts` 테이블 집계)
...
| 포함된 글         | 대분류 slug 기준 글 수         | 소분류 slug 기준 글 수         | DB 실시간 집계           |
| 다국어 지원 여부  | `지원` / `미지원` 표시         | `지원` / `미지원` 표시         | `is_multilingual` 기반   |
| 생성일            | 생성일 (YYYY-MM-DD)            | 생성일 (YYYY-MM-DD)            | `created_at` slice(0,10) |
...
```
대분류 카테고리명 | 소분류 카테고리명 | 포함된 글 | 다국어 지원 여부 | 생성일
──────────────────────────────────────────────────────────────────────────
맛집 (링크)      |                   |    28     | 지원             | 2026-01-05
...
- 대분류: 카테고리명 + 슬러그 생성 (SlugField) + 다국어 지원 체크박스 + 7개 언어 번역 입력 + "AI 카테고리 번역" 버튼
- 소분류: 대분류 선택 + 카테고리명 + 슬러그 생성 + 다국어 지원 체크박스 + 7개 언어 번역 입력 + "AI 카테고리 번역" 버튼
- **다국어 체크박스 경고 토스트 (CM-10)**: 체크박스 클릭 시 `toast.info` 표시 -- "한 번 설정하면 현재는 변경이 불가합니다. 추후 변경 기능이 지원될 예정입니다."
- 다국어 체크 시 7개 언어 번역 입력 필드 표시, "AI 카테고리 번역" 버튼으로 GPT-5 Mini 자동 번역 가능
- 저장 시 `createParentCategory` / `createChildCategory` Server Action 호출 (`isMultilingual`, `translations` 파라미터 포함)

**소분류 다국어 정합성 규칙**:

- `fetchParentCategories()`가 `{ id, slug, name, is_multilingual }[]`를 반환하여 대분류의 다국어 상태를 소분류 생성 폼에 전달
- 대분류가 다국어 미지원(`is_multilingual = false`)인 경우:
  - 소분류 다국어 체크박스가 disabled 처리 (체크 불가)
  - 안내 문구 표시: "대분류가 다국어 미지원이므로 소분류도 다국어를 지원할 수 없습니다."
- 대분류 선택을 변경할 때 새로 선택한 대분류가 다국어 미지원이면:
...
│   ├── constants/
│   │   └── prompts.ts               # GPT 프롬프트 중앙 관리 (5개: 요약, 슬러그, 용어추출, 번역, 카테고리번역)
│   ├── lib/
│   │   ├── supabase.ts              # Supabase 브라우저 클라이언트 (lazy init)
│   │   ├── supabase-server.ts       # Supabase 서버 클라이언트 (Service Role Key)
│   │   └── openai.ts                # OpenAI GPT-5 Mini 클라이언트 (브라우저 직접 호출)
│   └── types/
...
| Sheet       | sheet.tsx       | 모바일 시트 (터치 스와이프 닫기 지원) |
...
└── src/
    ...
    │   │   │   ├── image.ts
    │   │   │   ├── image-carousel.ts
    │   │   │   └── link-bookmark.ts       # CustomLinkBookmark 확장 (OG 태그 북마크 카드)
    │   │   ├── components/
    │   │   │   ├── TiptapEditor.tsx
    │   │   │   ├── TiptapEditorSkeleton.tsx
    │   │   │   ├── Toolbar.tsx
    │   │   │   ├── LinkPastePopup.tsx      # URL 붙여넣기 시 링크/북마크 선택 팝업
    │   │   │   ├── CategorySelector.tsx    # 카테고리/서브카테고리 select
    │   │   │   ├── ThumbnailUpload.tsx     # 썸네일 업로드 (WebP 변환)
    │   │   │   ├── VisitFields.tsx         # 체험 방문 전용 필드 (장소/주소/가격대)
    │   │   │   ├── ProductReviewFields.tsx # 제품 리뷰 전용 필드 (useFieldArray 동적 제품 목록)
    │   │   │   ├── PriceInputRow.tsx       # 가격 설명 + 금액 입력 공용 (visit/product 공용)
...
| `openai`                        | OpenAI SDK (GPT-5 Mini API 호출)            | 설치 완료 |
...
4. [번역] → is_multilingual === true인 경우
     ├─ Server Action: GPT-5 Mini API 호출 (7개 locale 병렬, Promise.allSettled)
