...
> Status: Phase 1 완료, Phase 3 Tiptap 에디터 + 이미지 삽입 + 폼 형식/메타 폼/번역/폼 검증(react-hook-form + Zod)/UX 구현 완료 (저장/S3 업로드 미구현), GPT-5 Nano API 실제 연동 완료 (mock 제거), 게시글 목록 페이지 구현 완료, Phase 5-1 (SearchFilter 리팩토링 + Pagination + 카테고리 관리) 진행 예정
...
/categories                → 카테고리 관리 (검색 + 그룹 테이블) — 미구현
```
...
└── SidebarLayout (app/sidebar-layout.tsx — 'use client')
    ...
    └── SidebarInset > main
        ...
        └── /categories     → CategoryListPage (카테고리 관리) — 미구현
```
...
| ID   | 요구사항                                                | 우선순위 |
| ---- | ------------------------------------------------------- | -------- |
| CM-1 | 카테고리 목록 테이블 (대분류-소분류 그룹 구조)          | P0       |
| CM-2 | 검색 필터 (카테고리명 검색 — SearchFilter.Query만 사용) | P1       |
| CM-3 | 새 카테고리 생성 버튼                                   | P1       |
| CM-4 | 다국어 지원 여부 표시                                   | P2       |

#### 카테고리 목록 페이지 (`/categories`)

- **라우팅**: 사이드바 "카테고리 생성/수정/삭제" → `/categories` (현재 `href: null` → 활성화)
...
| 컬럼              | 대분류 행          | 소분류 행              | 비고           |
| ----------------- | ------------------ | ---------------------- | -------------- |
| 대분류 카테고리명 | 대분류명 표시      | 비워둠                 | 그룹 헤더 역할 |
| 소분류 카테고리명 | 비워둠             | 소분류명 표시          |                |
...
└── src/
    ...
    │   │   └── constants/
    │   │       └── category.ts      # FORM_TYPE_OPTIONS, CATEGORY_OPTIONS, SUB_CATEGORY_MAP
    │   ├── category-management/      # 카테고리 관리 feature (미구현)
    │   │   ├── components/
