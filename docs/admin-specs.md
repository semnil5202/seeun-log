...
> Status: Phase 1 완료, Phase 3 Tiptap 에디터 + 이미지 삽입 + 폼 형식/메타 폼/번역/폼 검증(react-hook-form + Zod)/UX 구현 완료 (저장/S3 업로드 미구현), Phase 2 진행 예정
...
| 번역        | OpenAI GPT (CSR에서 직접 호출). GPT-5 Nano 전환 예정 (번역+요약)      |
...
| 제약                       | 이유                               | 대응                                           |
| -------------------------- | ---------------------------------- | ---------------------------------------------- |
| Vercel Hobby 10초 타임아웃 | Serverless Function 실행 시간 제한 | GPT 번역/요약을 CSR 브라우저에서 직접 호출     |
...
| `translation`     | GPT 다국어 번역 (GPT-5 Nano 예정)  | 에디터 내 사용                   | 부분 구현 (고유명사 추출 + 번역 + 미리보기 Sheet)                                 |
...
| PE-12 | 3줄 요약 AI 생성 (Server Action, GPT-5 Nano 예정)                                     | P0       | Mock 구현            |
...
- 3줄 요약: textarea + "요약 생성" 버튼 (Server Action `generateSummary` -- 현재 mock)
...
| 번역 (GPT-5 Nano 예정)               | CSR                                                      | Vercel Hobby 10초 타임아웃 회피 |
...
├── api/
│   └── actions.ts                   # ✅ Server Actions (generateSummary — 현재 mock)
├── containers/
...
| ID   | 요구사항                                                         | 우선순위 |
| ---- | ---------------------------------------------------------------- | -------- |
| TR-1 | GPT 번역 호출 (CSR 브라우저에서 직접). GPT-5 Nano 전환 예정      | P0       |
...
[브라우저]                                  [OpenAI API]
    ...
    │     ├─ model: GPT-5 Nano (예정, 현재 mock)│
    │     ├─ 시스템 프롬프트: 번역 지침         │
    │     └─ 유저 메시지: title + description + content + place_name + address
    │                                           │
    │<── 3. 번역 결과 수신 ─────────────────────┤
    │                                           │
...
- **모델**: GPT-5 Nano 사용 예정 (번역+요약 모두). 현재는 mock 상태.
...
| 작업                                      | 처리 위치     | 이유                           |
| ----------------------------------------- | ------------- | ------------------------------ |
| GPT API 호출 (GPT-5 Nano 예정)            | CSR           | Vercel 10초 타임아웃 회피      |
...
> **현재 구현 상태**: 고유명사 추출 + 번역 실행 + 미리보기 Sheet까지 구현 완료. 번역 결과 DB 저장(post_translations UPSERT)은 미구현. 에디터 페이지(`/posts/new`)에서 직접 연동하여 사용. GPT API는 아직 mock 상태 (GPT-5 Nano 전환 예정).
...
| `NEXT_PUBLIC_OPENAI_API_KEY`    | OpenAI API Key (CSR 번역+요약용, GPT-5 Nano 예정) | `sk-...`                       |
...
4. [번역] → is_multilingual === true인 경우
     ├─ CSR: GPT-5 Nano API 호출 (locale별 병렬, 예정)
...
| 3-1   | translation (부분) | 고유명사 추출, 번역 실행, 번역 미리보기 Sheet (에디터 페이지 내 연동)                                | P0       | **부분 구현 완료** (저장 미연동)                                 |
...
| 7     | translation (완성) | GPT-5 Nano 번역 저장, 번역 상태 관리                                                                 | P0       | 미착수                                                           |
